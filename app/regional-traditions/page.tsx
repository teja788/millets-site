...
export default function RegionalTraditionsPage() {
  ...
  useEffect(() => {
    document.title = 'Regional Millet Traditions of India | Millets Info';
  }, []);

  const filteredTraditions = useMemo(() => {
    if (activeRegion === 'all') return regionalTraditions;
    return regionalTraditions.filter((t) => t.region === activeRegion);
  }, [activeRegion]);
        ...
        {filteredTraditions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTraditions.map((tradition) => (
              <Link
                key={tradition.slug}
                href={`/regional-traditions/${tradition.slug}`}
              >
