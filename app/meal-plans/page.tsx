...
function MealPlanContent({ plan }: { plan: (typeof mealPlans)[number] }) {
  return (
    <div>
      ...
      <div className="mb-6">
        <p className="text-earth-200 leading-relaxed mb-3">
          {plan.description}
        </p>
        <p className="text-sm text-earth-300">
          <strong className="text-earth-100">Goal:</strong> {plan.goal}
        </p>
      </div>
      ...
      <div className="mt-6">
        ...
        <ul className="space-y-2">
          {plan.tips.map((tip, index) => (
            <li key={index} className="flex items-start gap-3 text-earth-200 text-sm">
              <span className="w-2 h-2 rounded-full bg-earth-400 flex-shrink-0 mt-1.5" />
      ...
      {/* Plan Disclaimer */}
      <div className="mt-6 bg-earth-800 rounded-lg p-4">
